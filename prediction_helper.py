...
def prepare_input(age, income, loan_amount, loan_tenure_months, avg_dpd_per_delinquency,
    ...
    #Ensure only required columns for scaling are required
...
def predict(age, income, loan_amount, loan_tenure_months, avg_dpd_per_delinquency,
            delinquency_ratio, credit_utilization_ratio, num_open_accounts, residence_type, loan_purpose, loan_type):
...
def calculate_credit_score(input_df, base_score=300, scale_length=600):
    ...
    #Raw score(x) calculation
    x = np.dot(input_df.values,model.coef_.T) + model.intercept_ #.T for doing transpose of an array

    # Apply the logistic function(Activation function) to get the probability
    default_probability = 1 / (1 + np.exp(-x))

    non_default_probability = 1 - default_probability

    #Convert the probability to a credit score, scaled to fit within 300 to 900
    credit_score = base_score + non_default_probability.flatten() * scale_length #flatten() is used to  ensure the probability is in a compatible format for multiplication.
